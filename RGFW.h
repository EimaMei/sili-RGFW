...
#ifndef SGFW_INCLUDE_SGFW_H
#define SGFW_INCLUDE_SGFW_H
...
extern "C" {
	...
	#define RGFWDEF SIDEF
		...
		#if SI_LANGUAGE_IS_CPP == 0
			#define __uuidof(T) IID_##T
		#endif
	#endif

	#define RGFW_WINDOWS 1
	...
	#define RGFW_WEBASM 1

#endif


#if !defined(RGFW_EGL) && (defined(RGFW_OPENGL_ES1) || defined(RGFW_OPENGL_ES2) || defined(RGFW_OPENGL_ES3))
	#define RGFW_EGL 1
#endif

#if !defined(RGFW_OSMESA) && !defined(RGFW_EGL) && !defined(RGFW_OPENGL) && !defined(RGFW_DIRECTX) && !defined(RGFW_BUFFER) && !defined(RGFW_NO_API)
	#define RGFW_OPENGL 1
#endif
...
#ifndef RGFW_ALLOC
	#define RGFW_ALLOC(userptr, size) (RGFW_UNUSED(userptr), si_malloc(size))
	#define RGFW_FREE(userptr, ptr) (RGFW_UNUSED(userptr),si_mfree(ptr))
#endif

#ifndef RGFW_MEMCPY
	#define RGFW_MEMCPY(dist, src, len) si_memcopy(dist, src, len)
	//#define RGFW_STRNCMP(s1, s2, max) strncmp(s1, s2, max)
	//#define RGFW_STRTOL(str, endptr, base) strtol(str, endptr, base)
#endif

#ifndef RGFW_BIT
	#define RGFW_BIT(x) SI_BIT(x)
#endif
...
#define SGFW_CHECK(condition, code) \
	SI_ERROR_CHECK_EX(condition, code, SGFW_errorLog, nil, return SI_ERROR_RES)

#define SGFW_CHECK_EX(condition, code, ...) \
	SI_ERROR_CHECK_EX(condition, code, SGFW_errorLog, nil, __VA_ARGS__)


#define SGFW_CHECK_PLATFORM(condition, platformCode) \
	SI_ERROR_CHECK_EX_RET(condition, SGFW_errorPlatform, SGFW_errorLog, &(i32){platformCode})
...
*/
typedef void* (*RGFW_allocatorMallocfunc)(void* userdata, isize size);
...
RGFWDEF void* RGFW_alloc(isize len);
...
#ifndef RGFW_point
	typedef siPoint RGFW_point;
#endif

/*! basic rect type, if there's not already a rect type of choice */
#ifndef RGFW_rect
	typedef siRect RGFW_rect;
#endif

/*! basic area type, if there's not already a area type of choice */
#ifndef RGFW_area
	typedef siArea RGFW_area;
#endif

#define RGFW_POINT(x, y) SI_POINT(x, y)
#define RGFW_RECT(x, y, w, h) SI_RECT(x, y, w, h)
#define RGFW_AREA(w, h) SI_AREA(w, h)
	...
	typedef struct RGFW_monitor {
		...
		f32 scaleX, scaleY; /*!< monitor content scale*/
		f32 pixelRatio; /*!< pixel ratio for monitor (1.0 for regular, 2.0 for)  */
		f32 physW, physH; /*!< monitor physical size in inches*/
	} RGFW_monitor;
...
typedef struct RGFW_Event {
...
#endif
	usize droppedFilesCount; /*!< house many files were dropped */
	...
	f64 scroll; /*!< the raw mouse scroll value */
...
*/
RGFWDEF void RGFW_setClassName(siString name);
...
RGFWDEF RGFW_window* RGFW_createWindow(
	siString name, /* name of the window */
	RGFW_rect rect, /* rect of window */
	RGFW_windowFlags flags /* extra arguments */
); /*!< function to create a window and struct */

RGFWDEF siError RGFW_createWindowPtr(
	siString name, /* name of the window */
	RGFW_rect rect, /* rect of window */
	RGFW_windowFlags flags, /* extra arguments */
...
RGFWDEF void RGFW_window_setMousePassthrough(RGFW_window* win, b32 passthrough);
#endif
...
RGFWDEF b32 RGFW_window_setMouse(RGFW_window* win, RGFW_mouse* mouse);
...
this is run by default if the user uses the arg `RGFW_windowScaleToMonitor` during window creation
*/
...
RGFWDEF RGFW_gamepadButtonfunc RGFW_setgpButtonCallback(RGFW_gamepadButtonfunc func);
/*! set callback for a gamepad axis mov event returns previous callback function (if it was set)  */
RGFWDEF RGFW_gamepadAxisfunc RGFW_setgpAxisCallback(RGFW_gamepadAxisfunc func);
...
RGFWDEF i32 RGFW_window_checkFPS(RGFW_window* win, i32 fpsCap);
...
RGFWDEF RGFW_key RGFW_apiKeyToRGFW(i32 keycode);
...
void* RGFW_allocatorMalloc(void* userdata, isize size) { return si_malloc(size);  }
void RGFW_allocatorFree(void* userdata, void* ptr) { si_mfree(ptr); }
RGFW_allocator RGFW_current_allocator = {nil, RGFW_allocatorMalloc, RGFW_allocatorFree};
...
void* RGFW_alloc(isize len) { return RGFW_current_allocator.alloc(RGFW_current_allocator.userdata, len); }
...
RGFW_key RGFW_apiKeyToRGFW(i32 keycode) {
	SI_ASSERT_NOT_NEG(keycode);
	SI_ASSERT(keycode < countof(RGFW_keycodes));
...
void RGFW_resetKey(void) {
	si_memset(RGFW_keyboard, 0, si_sizeof(RGFW_keyboard));
}
...
RGFW_gamepadButtonfunc RGFW_setgpButtonCallback(RGFW_gamepadButtonfunc func) {
...
RGFW_gamepadAxisfunc RGFW_setgpAxisCallback(RGFW_gamepadAxisfunc func) {
...
RGFW_window* RGFW_createWindow(siString name, RGFW_rect rect, RGFW_windowFlags flags) {
	RGFW_window* win = RGFW_alloc(sizeof(RGFW_window));
	win->_flags |= RGFW_WINDOW_ALLOC;

	siError error = RGFW_createWindowPtr(name, rect, flags, win);
	if (error.code != 0) {
		RGFW_free(win);
		return nil;
	}
...
void RGFW_window_setBufferPtr(RGFW_window* win, u8* ptr, RGFW_area size) {
	...
	SI_ASSERT_NOT_NEG(size.width);
	SI_ASSERT_NOT_NEG(size.height);
...
void RGFW_window_scaleToMonitor(RGFW_window* win) {
	...
	f32 x = (f32)win->r.width * monitor.scaleX,
		y = (f32)win->r.height * monitor.scaleY;
	RGFW_window_resize(win, RGFW_AREA((i32)x, (i32)y));
}
...
void RGFW_window_mouseHold(RGFW_window* win, RGFW_area area) {
	SI_ASSERT_NOT_NIL(win);
	SI_ASSERT_NOT_NEG(area.width);
	SI_ASSERT_NOT_NEG(area.height);

	if ((win->_flags & RGFW_HOLD_MOUSE))
		return;

	siArea half = RGFW_AREA(win->r.width / 2, win->r.height / 2);
	if (!area.width && !area.height)
		area = half;

	win->_flags |= RGFW_HOLD_MOUSE;
	RGFW_captureCursor(win, win->r);
	RGFW_window_moveMouse(win, RGFW_POINT(win->r.x + half.width, win->r.y + half.height));
}
...
i32 RGFW_window_checkFPS(RGFW_window* win, i32 fpsCap) {
	SI_ASSERT_NOT_NIL(win);

	i32 deltaTime = (i32)(si_clock() - win->event.frameTime);
	i32 fps = SI_CLOCKS_SECS / deltaTime;

	if (fpsCap > 0 && fps > fpsCap) {
		i32 frameTime = (SI_CLOCKS_SECS / fpsCap) - deltaTime;

		if (frameTime >= SI_CLOCKS_MILI) {
			frameTime /= SI_CLOCKS_MILI;
			si_sleep(frameTime);
			win->event.frameTime = 0;
		}
	}

	win->event.frameTime = si_clock();

	if (fpsCap <= 0)
		return fps;

	u64 frameTime2 = win->event.frameTime2;
	win->event.frameTime2 = si_clock();

	return (i32)(win->event.frameTime2 - frameTime2);
}
...
#if 1 /* === CUSTOM SGFW CODE IMPLEMENTATION STARTS HERE === */
...
siString SGFW_errorName(SGFW_error error) {
    ...
    static const siString names[SGFW_errorLength] = {
		...
		SI_STRC("SGFW_errorGlxConfig"),
		SI_STRC("SGFW_errorGlxVisual"),
		#endif
    };
...
#endif /* === CUSTOM SGFW CODE IMPLEMENTATION ENDS HERE === */
...
i32* RGFW_initFormatAttribs(b32 useSoftware) {
	...
	isize index = countof(attribs) - 13;
	...
	if (RGFW_majorVersion >= 4 || RGFW_majorVersion >= 3) {
		attribs[index + 1] = (RGFW_majorVersion >= 4) ? 0x4100 : 0x3200;
	}
	#endif
...
#if !defined(RGFW_NO_X11_CURSOR) && !defined(RGFW_NO_X11_CURSOR_PRELOAD)
	PFN_XcursorImageLoadCursor XcursorImageLoadCursorSrc;
	PFN_XcursorImageCreate XcursorImageCreateSrc;
	PFN_XcursorImageDestroy XcursorImageDestroySrc;
...
void RGFW_captureCursor(RGFW_window* win, RGFW_rect r) {
	...
	RGFW_window_moveMouse(win, RGFW_POINT(win->r.x + (r.width / 2), win->r.y + (r.height / 2)));
}


#ifdef __CYGWIN__
	#define RGFW__LIBXCURSOR "libXcursor-1.so"
	#define RGFW__LIBXI "libXi-6.so"
	#define RGFW__LIBXEXT "libXext-6.so"
#elif defined(__CYGWIN__)
	#define RGFW__LIBXCURSOR "libXcursor.so"
	#define RGFW__LIBXI "libXi.so"
	#define RGFW__LIBXEXT "libXext.so"
#else
	#define RGFW__LIBXCURSOR "libXcursor.so.1"
	#define RGFW__LIBXI "libXi.so.6"
	#define RGFW__LIBXEXT "libXext.so.6"
#endif

#define RGFW_PROC_DEF(proc, name) name = si_dllProcAddressFunc(proc, #name, PFN_##name)

siError RGFW_createWindowPtr(siString name, RGFW_rect rect, RGFW_windowFlags flags,
		RGFW_window* win) {
	#ifdef RGFW_USE_XDL
	XDL_init();
	#endif
	...
	if (X11Cursorhandle == NULL) {
		X11Cursorhandle = si_dllLoad(RGFW__LIBXCURSOR);
	...
	#if !defined(RGFW_NO_X11_XI_PRELOAD)
	if (X11Xihandle == NULL) {
		X11Xihandle = si_dllLoad(RGFW__LIBXI);
	...
	if (flags & RGFW_windowOpenglSoftware)
		si_envVarSet(SI_STR("LIBGL_ALWAYS_SOFTWARE"), SI_STR("1"));

	RGFW_window_basic_init(win, rect, flags);

	i64 event_mask = KeyPressMask | KeyReleaseMask | ButtonPressMask | ButtonReleaseMask
		| PointerMotionMask | StructureNotifyMask | FocusChangeMask | LeaveWindowMask
		| EnterWindowMask | ExposureMask;

	#ifdef RGFW_OPENGL
		i32* visual_attribs = RGFW_initFormatAttribs(flags & RGFW_windowOpenglSoftware);
		i32 fbcount;
		GLXFBConfig* fbc = glXChooseFBConfig(
			win->src.display, DefaultScreen(win->src.display), visual_attribs, &fbcount
		);
		SGFW_CHECK_EX(fbcount == 0, SGFW_errorGlxConfig, XFree(fbc); return SI_ERROR_RES);

		i32 best_fbc = -1;
		for_range (i, 0, fbcount) {
			XVisualInfo* vi = glXGetVisualFromFBConfig(win->src.display, fbc[i]);
			SI_STOPIF(vi == NULL, continue);
			XFree(vi);
		...
		SGFW_CHECK_EX(best_fbc == -1, SGFW_errorGlxVisual, XFree(fbc); return SI_ERROR_RES);
	...
	Colormap cmap = XCreateColormap(
		win->src.display, DefaultRootWindow(win->src.display), vi->visual, AllocNone
	);
	...
	win->src.window = XCreateWindow(
		...
		win->r.x, win->r.y, (u32)win->r.width, (u32)win->r.height,
	...
	if ((flags & RGFW_windowNoInitAPI) == 0) {
		...
		i32 context_attribs[7] = {
			GLX_CONTEXT_PROFILE_MASK_ARB,
			(RGFW_profile) ? GLX_CONTEXT_CORE_PROFILE_BIT_ARB : GLX_CONTEXT_COMPATIBILITY_PROFILE_BIT_ARB,
			0, 0, 0, 0, 0
		};
		...
		glXCreateContextAttribsARB = si_transmute(
			glXCreateContextAttribsARBProc,
			glXGetProcAddressARB((u8*)"glXCreateContextAttribsARB"),
			__GLXextFuncPtr
		);

		GLXContext ctx = (RGFW_root != NULL) ? RGFW_root->src.ctx : NULL;
		win->src.ctx = glXCreateContextAttribsARB(
			win->src.display, bestFbc, ctx, True, context_attribs
		);
	#endif
	...
	if (flags & RGFW_windowCenter) {
		...
		RGFW_window_move(
			win,
			RGFW_POINT((screenR.width - win->r.height) / 2, (screenR.height - win->r.height) / 2)
		);
	}
	...
	if (flags & RGFW_windowNoResize) {
		XSizeHints sh = {0};
		sh.flags = (1L << 4) | (1L << 5);
		sh.min_width = sh.max_width = win->r.width;
		sh.min_height = sh.max_height = win->r.height;

		XSetWMSizeHints(win->src.display, win->src.window, &sh, XA_WM_NORMAL_HINTS);
	...
	XSelectInput(win->src.display, win->src.window, event_mask); /*!< tell X11 what events we want*/
	...
	XSetWMProtocols(win->src.display, win->src.window, &wm_delete_window, 1);
	...
	if (flags & RGFW_windowAllowDND) {
		...
		XChangeProperty(
			win->src.display, win->src.window,
			XdndAware, 4, 32, PropModeReplace, &version, 1
...
#undef RGFW__LIBXCURSOR
#undef RGFW__LIBXI


RGFW_area RGFW_getScreenSize(void) {
	SI_ASSERT_NOT_NIL(RGFW_root);
...
RGFW_point RGFW_getGlobalMousePoint(void) {
	...
	XQueryPointer(
		RGFW_root->src.display, XDefaultRootWindow(RGFW_root->src.display),
		&window1, &window2, &RGFWMouse.x, &RGFWMouse.y, &x, &y, &z
	);
...
RGFW_point RGFW_window_getMousePoint(RGFW_window* win) {
	...
	XQueryPointer(
		win->src.display, win->src.window,
		&window1, &window2, &x, &y, &RGFWMouse.x, &RGFWMouse.y, &z
	);
...
RGFW_Event* RGFW_window_checkEvent(RGFW_window* win) {
	...
	case KeyRelease: {
		...
		/* check if it's a real key release and if so, get next event, if present. */
		if (E.type == KeyRelease && XEventsQueued(win->src.display, QueuedAfterReading)) {
			XEvent NE;
			XPeekEvent((Display*) win->src.display, &NE);

			/* check if the current and next are both the same*/
			if (E.xkey.time == NE.xkey.time && E.xkey.keycode == NE.xkey.keycode)
				win->event.repeat = RGFW_TRUE;
		}

		/* TODO(EimaMei): Cast warning here for 'KeyCode'. Investigate? */
		KeySym sym = XkbKeycodeToKeysym(
			win->src.display, (KeyCode)E.xkey.keycode, 0, (E.xkey.state & ShiftMask) != 0
		);
		...
		win->event.key = RGFW_apiKeyToRGFW((u32)E.xkey.keycode);
		...
		RGFW_keyCallback(
			win, win->event.key, win->event.keyChar,
	...
	case MotionNotify: {
		...
		win->event.point = RGFW_POINT(E.xmotion.x, E.xmotion.y);

		if ((win->_flags & RGFW_HOLD_MOUSE)) {
			win->event.point.y = E.xmotion.y;

			win->event.point.x -= win->_lastMousePoint.x;
			win->event.point.y -= win->_lastMousePoint.y;
		}
	...
	case GenericEvent: {
		...
		if (E.xcookie.evtype == XI_RawMotion) {
			...
			f64 deltaX = (XIMaskIsSet(raw->valuators.mask, 0))
				? raw->raw_values[0]
				: 0.0;
			f64 deltaY = (XIMaskIsSet(raw->valuators.mask, 1))
				? raw->raw_values[1]
				: 0.0;
			...
			RGFW_window_moveMouse(
				win,
				RGFW_POINT(win->r.x + (win->r.width / 2), win->r.y + (win->r.height / 2))
			);
	...
	case SelectionNotify: {
		...
		/* TODO(EimaMei): Rework this because this looks awful */
		...
		if (xdnd.version >= 2) {
			...
			reply.xclient.data.l[2] = (long)XdndActionCopy;
...
void RGFW_window_resize(RGFW_window* win, RGFW_area a) {
	...
	win->r.width = a.width;
	win->r.height = a.height;
	XResizeWindow(win->src.display, win->src.window, (u32)a.width, (u32)a.height);
	...
	XSizeHints sh = {0};
	sh.flags = (1L << 4) | (1L << 5);
	sh.min_width = sh.max_width = a.width;
	sh.min_height = sh.max_height = a.height;
...
void RGFW_window_setMinSize(RGFW_window* win, RGFW_area a) {
	...
	hints.min_width = a.width;
	hints.min_height = a.height;
...
void RGFW_window_setMaxSize(RGFW_window* win, RGFW_area a) {
	...
	hints.max_width = a.width;
	hints.max_height = a.height;
	XSetWMNormalHints(win->src.display, win->src.window, &hints);
}
...
siDllProc RGFW_libxshape = NULL;

#ifndef RGFW_NO_PASSTHROUGH
void RGFW_window_setMousePassthrough(RGFW_window* win, b32 passthrough) {
	SI_ASSERT_NOT_NIL(win);
	...
	if (RGFW_libxshape == nil) {
		RGFW_libxshape = si_dllLoad(RGFW__LIBXEXT);
...
b32 RGFW_window_setMouse(RGFW_window* win, RGFW_mouse* mouse) {
	SI_ASSERT_NOT_NIL(win);
	SI_ASSERT_NOT_NIL(mouse);
	return (b32)XDefineCursor(win->src.display, win->src.window, (Cursor)mouse);
}
...
b32 RGFW_window_setMouseStandard(RGFW_window* win, u8 mouse) {
	SI_ASSERT_NOT_NIL(win);
	SI_ASSERT(mouse < countof(RGFW_mouseIconSrc));

	Cursor cursor = XCreateFontCursor(win->src.display, RGFW_mouseIconSrc[mouse]);
...
siString RGFW_readClipboard(siBuffer out) {
	...
	XConvertSelection(
		RGFW_root->src.display, RGFW_XCLIPBOARD, RGFW_XUTF8_STRING, XSEL_DATA,
		RGFW_root->src.window, CurrentTime
	);
	XSync(RGFW_root->src.display, 0);
	XNextEvent(RGFW_root->src.display, &event);

	if (event.type != SelectionNotify || event.xselection.selection != RGFW_XCLIPBOARD
	...
	SI_STOPIF(target != RGFW_XUTF8_STRING && target != XA_STRING, XFree(data); return SI_STR_EMPTY);
...
isize RGFW_readClipboardLen(void) {
	...
	*/
	...
	static void XGetSystemContentScale(Display* display, f32* xscale, f32* yscale) {
...
- window flags:
	...
	#define RGFW_windowScaleToMonitor 			scale the window to the screen
...
static const struct wl_callback_listener wl_surface_frame_listener = {
	...
	RGFW_window* RGFW_createWindow(const char* name, RGFW_rect rect, u16 flags) {
	...
	RGFW_Event* RGFW_window_checkEvent(RGFW_window* win) {
		if (win->_flags & RGFW_WINDOW_HIDE)
			return NULL;
...
RGFW_UNUSED(win); /*!< if buffer rendering is not being used */
	...
	RGFW_window* RGFW_createWindow(const char* name, RGFW_rect rect, u16 flags) {
		...
		if (wglCreateContextAttribsARB != NULL) {
			...
			if (wglChoosePixelFormatARB != NULL) {
				i32* pixel_format_attribs = (i32*)RGFW_initFormatAttribs(flags & RGFW_OPENGL_SOFTWARE);
	...
	static i32 RGFW_checkXInput(RGFW_window* win, RGFW_Event* e) {
		...
		for (i = 0; i < 4; i++) {
			...
			if ((keystroke.Flags & XINPUT_KEYSTROKE_REPEAT) == 0 && result != ERROR_EMPTY) {
				...
				//gp + 1 = RGFW_gamepadButtonReleased
...
#define INPUT_DEADZONE  ( 0.24f * (f32)(0x7FFF) )  // Default to 24% of the +/- 32767 range.   This is a reasonable default value but can be altered if needed.
			...
			e->axisesCount = 2;
			RGFW_point axis1 = RGFW_POINT(((f32)state.Gamepad.sThumbLX / 32768.0f) * 100, ((f32)state.Gamepad.sThumbLY / -32768.0f) * 100);
			RGFW_point axis2 = RGFW_POINT(((f32)state.Gamepad.sThumbRX / 32768.0f) * 100, ((f32)state.Gamepad.sThumbRY / -32768.0f) * 100);
	...
	RGFW_monitor win32CreateMonitor(HMONITOR src) {
		...
		if (GetDpiForMonitor != NULL) {
			...
			monitor.scaleX = (f32) (x) / (f32) USER_DEFAULT_SCREEN_DPI;
			monitor.scaleY = (f32) (y) / (f32) USER_DEFAULT_SCREEN_DPI;
		}
#endif
		...
		monitor.physW = GetSystemMetrics(SM_CYSCREEN) / (f32) ppiX;
		monitor.physH = GetSystemMetrics(SM_CXSCREEN) / (f32) ppiY;
...
#define NS_OPENGL_ENUM_DEPRECATED(minVers, maxVers) API_AVAILABLE(macos(minVers))
	...
	*/
...
#ifdef RGFW_OPENGL
	if ((flags & RGFW_windowNoInitAPI) == 0) {
		void* attrs = RGFW_initFormatAttribs(flags & RGFW_OPENGL_SOFTWARE);
...
#endif
	{
		...
		if (flags & RGFW_COCOA_MOVE_TO_RESOURCE_DIR)
			NSMoveToResourceDir();
	...
	RGFW_monitor RGFW_NSCreateMonitor(CGDirectDisplayID display) {
		...
		monitor.physW = (f32)screenSizeMM.width / 25.4f;
		monitor.physH = (f32)screenSizeMM.height / 25.4f;


		f32 dpi_width = round((double)monitor.rect.w/(double)monitor.physW);
		f32 dpi_height = round((double)monitor.rect.h/(double)monitor.physH);
...
void RGFW_window_eventWait(RGFW_window* win, i32 waitMS) {
	...
	u32 start = (u32)(((u64)si_clock()) / 1e+6);

	while ((RGFW_eventLen == 0) && RGFW_stopCheckEvents_bool == RGFW_FALSE &&
		(waitMS < 0 || (si_clock() / 1e+6) - start < waitMS)
	) {
		emscripten_sleep(0);
	}
...
void RGFW_window_swapBuffers(RGFW_window* win) {
	...
	if (!(win->_flags & RGFW_NO_CPU_RENDER)) {
		...
		f32 ratioX = ((f32)win->r.w / (f32)RGFW_bufferSize.w);
		f32 ratioY = ((f32)win->r.h / (f32)RGFW_bufferSize.h);
